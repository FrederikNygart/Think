...
namespace Think
{
    public class NeuralNetwork
    {
        ...
        public List<double> FeedForward(List<double> input)
        {
            ...
            var hidden = Matrix.MatrixProduct(WeightsInputToHidden.Data, inputs);
            var hiddenWithBias = Matrix.Add(hidden, BiasHidden.Data);
            HiddenProduct = Matrix.Map(hiddenWithBias, Sigmoid);

            var output = Matrix.MatrixProduct(WeightsHiddenToOutput.Data, HiddenProduct);
        ...
        public double[,] Train(List<double> input, List<double> targets)
        {
            ...
            var hiddenError = Matrix.MatrixProduct(transposedHtOWeights, outputError);
            ...
            //Matrix.Print(WeightsInputToHidden.Data);
            //Matrix.Print(WeightsHiddenToOutput.Data);
        ...
        private double[,] CalcDelta(double[,] cause, double[,] result, double[,] error)
        {
            ...
            gradients = Matrix.HadamardProduct(gradients, error);
            gradients = Matrix.ScalarProduct(gradients, LearningRate);
            var transposedCause = Matrix.Transpose(cause);
            return Matrix.MatrixProduct(gradients, transposedCause);
        }
